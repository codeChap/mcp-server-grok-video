...
const GROK_VIDEO_GENERATIONS_URL: &str = "https://api.x.ai/v1/videos/generations";
const GROK_VIDEO_EDITS_URL: &str = "https://api.x.ai/v1/videos/edits";
...
struct VideoGenerationRequest {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    reference_images: Option<Vec<ImageRef>>,
}
...
struct GenerateVideoParams {
    ...
    prompt: String,
    #[schemars(description = "Video duration in seconds (1-15, default 6)")]
    duration: Option<u8>,
    #[schemars(
...
struct AnimateImageParams {
    ...
    duration: Option<u8>,
    #[schemars(
        description = "Aspect ratio. Options: 1:1, 16:9, 9:16, 4:3, 3:4, 3:2, 2:3. If omitted, defaults to the input image's aspect ratio."
    )]
    aspect_ratio: Option<String>,
    #[schemars(description = "Output resolution. Options: \"480p\" (default), \"720p\"")]
    resolution: Option<String>,
...
#[derive(Debug, Deserialize, JsonSchema)]
struct GenerateWithReferencesParams {
    #[schemars(
        description = "List of reference image URLs, base64 data URIs, or local file paths (1 or more) for style/subject guidance"
    )]
...
struct EditVideoParams {
    #[schemars(
        description = "request_id of a previously generated xAI video to edit. The /edits endpoint only re-edits videos already generated by xAI; it does not accept user-uploaded videos."
    )]
    video_id: String,
...
impl GrokVideoServer {
    /// Submit a video generation/edit request and poll until completion.
    async fn submit_and_poll<T: Serialize>(
        &self,
        submit_url: &str,
        body: &T,
    ) -> Result<VideoPollResponse, String> {
        debug!(submit_url, "Submitting video request");
        ...
        loop {
            ...
            match result.status.as_str() {
                ...
                "failed" => {
                    return Err(format!(
                        "Video generation failed (request_id: {})",
                        submit.request_id
                    ));
                }
                _ => {
...
impl GrokVideoServer {
    ...
    async fn generate_video(
        ...
        match self.submit_and_poll(GROK_VIDEO_GENERATIONS_URL, &request).await {
            Ok(resp) => {
    ...
    async fn animate_image(
        ...
        let request = VideoGenerationRequest {
            ...
            aspect_ratio: params.aspect_ratio,
            resolution: params.resolution,
            image: Some(ImageRef { url: image_url }),
            reference_images: None,
        };
    ...
    #[tool(
        description = "Generate a video guided by one or more reference images (style/subject guidance, NOT a starting frame). Use animate_image instead if you want the image to be the first frame."
    )]
    async fn generate_with_references(
        &self,
        Parameters(params): Parameters<GenerateWithReferencesParams>,
    ) -> Result<CallToolResult, McpError> {
        if let Err(e) = validate_duration(params.duration) {
            return Ok(CallToolResult::error(vec![Content::text(e)]));
        }
        ...
        if params.reference_images.is_empty() {
            return Ok(CallToolResult::error(vec![Content::text(
                "reference_images must contain at least one image".to_string(),
            )]));
        }

        let mut refs = Vec::with_capacity(params.reference_images.len());
        for r in &params.reference_images {
            match resolve_url(r).await {
                Ok(url) => refs.push(ImageRef { url }),
                Err(e) => return Ok(CallToolResult::error(vec![Content::text(e)])),
            }
        }
        ...
        info!(
            ...
            prompt = params.prompt,
            n_refs = refs.len(),
            "generate_with_references called"
        );
    ...
    #[tool(
        description = "Edit a previously generated xAI video using natural language instructions. Takes the request_id of an earlier generation (NOT a user-uploaded video) and a prompt describing the change."
    )]
    async fn edit_video(
        &self,
        ...
        let model = params.model.unwrap_or_else(|| DEFAULT_MODEL.to_string());
        info!(model, prompt = params.prompt, video_id = params.video_id, "edit_video called");
        ...
        match self.submit_and_poll(GROK_VIDEO_EDITS_URL, &request).await {
            Ok(resp) => {
                let contents = self.format_video_response(&resp).await;
                Ok(CallToolResult::success(contents))
            }
...
impl ServerHandler for GrokVideoServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            ...
            .with_instructions(
                "Grok video generation server. Use generate_video for text-to-video, \
                 animate_image to use a still image as the first frame, \
                 generate_with_references for style/subject guidance from one or more reference images, \
                 or edit_video to modify a previously generated xAI video by its request_id."
            )
    }
}
